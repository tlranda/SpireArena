...
'''
	...
	def Scrape(self):
		if self.Color == 'Red':
			...
			assert False, "Error: Blue Slaver cannot use Scrape!"
		# Deal damage
		dealt, targets = self.Damage(damage)
		# How you would do a multihit:
		#damages = [damage, damage]
	...
	def Entangle(self):
		#Entangle stops target from using attack in
		if self.Color == 'Red':
			entangle = 1
		else: # self.Color == 'Blue':
			assert False, "Error: Blue Slaver cannot use Entangle!"
...
class JawWorm(arena.Monster):
	...
	def __init__(self, ID="<TemplateJawWorm>", Arena=None, Friendlies=None, ascension=0, variant=0):
		# Ensure proper type
		ascension, variant = int(ascension), int(variant)
		# Constructor
		super().__init__(ID, Arena, Friendlies)
		self.Name = f"Jaw Worm"
->important		self.variant? = 2
		self.Act = 1
		# Stats
...
'''
	...
	def Thrash(self):
		...
		dealt, targets = self.Damage(damage)
		print(f"{str(self)} uses Thrash to deal RAW_INTENT:{damage} --> ACTUAL:{dealt} damage to you and gains 5 block!")
...
class Cultist(arena.Monster):
	...
	def __init__(self, ID="<TemplateSlaver>", Arena=None, Friendlies=None, ascension=0, variant=0):
		# Ensure proper type
		ascension, variant = int(ascension), 1
		# Constructor
		super().__init__(ID, Arena, Friendlies)
		self.Name = f"Cultist"
		self.Act = 1
		...
		# Powers
		# self.PowerPool.append(Power(Incantation)) # stops target from attacking for one turn]
		# Ascend the mortal form
		self.ascension = ascension
		if ascension > 0:
			self.Ascend(ascension)
...
class Louse(arena.Monster):
	...
	def __init__(self, ID="<TemplateSlaver>", Arena=None, Friendlies=None, ascension=0, variant=0):
		...
		# Powers
		# self.PowerPool = [Power(Curl Up) # stops target from attacking for one turn]
		# Ascend the mortal form
		self.ascension = ascension
		if ascension > 0:
			self.Ascend(ascension)
